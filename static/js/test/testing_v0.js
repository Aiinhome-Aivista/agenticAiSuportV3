...
const handleOnAiChat = () => {
  ...
  async function handleConfirmationAudio(transcript) {
    ...
    const { summary: greeting, audio_path, name, companies, email, error } =
      await greetingAIResponse(transcript);
    const end = greeting.indexOf(","); // find index of comma after name
    const hiName = greeting.substring(0, end); // "Hi Ravi"
    // console.log(hiName); // Output: "Hi Ravi"
    if (!error) {
      sessionStorage.setItem("userName", hiName);
      ...
      customToastUI(greeting, "error");
    }
  }
  // after confermation of name & email, this function will be called ===> regular API call
  async function regularApiCall(transcript) {
    lastChat[0] = transcript;
    if (transcript.includes("not satisfied") ||
      transcript.includes("not satisfy")
      || transcript.includes("not happy") ||
      transcript.includes("need help") ||
      transcript.includes("help") ||
      transcript.includes("need more") ||
      transcript.includes("session has not") ||
      transcript.includes("session")
    ) {
      console.log("User is not satisfied, raising ticket...");
      appendAIMessageWithPopup("Would you like me to escalate this to the technical team and raise a support ticket for you?", new Date().toTimeString().split(" ")[0]);
      return;
    }
    // If user wants to raise a ticket or continue chat
    if (transcript.includes("raise ticket") || transcript.includes("yes") || transcript.includes("ticket")) {
      // console.log("User wants to raise a ticket or continue chat.");

      // customToastUI("Thanks for your response. Ticket raised successfully. " + lastChat[0], "success");
      // return;
      const raiseTicketResponse = await handleRaiseTicket()
      if (!raiseTicketResponse) {
        customToastUI("Failed to raise ticket. Please try again later.", "error");
        return;
      }
      trackAiSpeechPlayback(api_ai_speech);
      customToastUI("Thanks for your response. Ticket raised successfully.", "success");
      return;

    }

    // console.log("Regular API call with transcript:", transcript);
    // If language selected and normal chat interaction
    try {
      const { summary: aiResponse, matched: status, audio_file, error } = await generateAIResponse(transcript);
      const now2 = new Date();
      const timeString2 = now2.toTimeString().split(" ")[0];

      if (!aiResponse || error) {
        isRegularApiError = true;
      ...
      isRegularApiError = true;
      customToastUI("Error generating AI response: " + err.message, "error");
      return;
    }
  }
  ...
  function attachRecognitionEvents() {
    ...
    recognition.addEventListener("result", async (event) => {
      ...
      if (firstInteraction) {
        ...
        // === REGULAR CHAT PHASE ===
        await regularApiCall(transcript);
        // await handleUserSatisfactionAndRaiseTicket(transcript);
  ...
  // This function is called when the user clicks on the "Raise Ticket" button
  ...
  // This function is called when the user clicks on the "Resolve" button
  ...
  // This function is called when the user clicks on the "Continue Chat" button
  ...
  function appendAIMessageWithPopup(message, time) {
    console.log("AI Message:", message);
    const msg = document.createElement("div");
    msg.className = "ai-chat-main";
    msg.innerHTML = `
  ...
  // Function to create chat options UI
  // This function is used to show options like "Raise Ticket", "Resolve", and "Continue Chat"
  function chatOptionUI() {
    return `
    <div class="ticket-pop-up flex flex-col space-y-2 relative grow-1">
  ...
  async function handleRaiseTicket() {
    ...
    try {
      ...
      if (!getSessionEmail) {
        ...
        return {
          response: "Session data for email is missing.",
          matched: false,
          audio_file: null,
        };
      }
      ...
      const response = await fetch(
        `${API_BASE_URL}${endpointConfig.raiseTicket}`,
        {
          method: "POST",
          headers: {
  ...
  async function askAiByApi(userText) {
    try {
      ...
      if (!getSessionEmail || !getSessionCompanies) {
        // console.warn("Session data for email or companies is missing.");
        customToastUI("missing data", "error");
        return {
